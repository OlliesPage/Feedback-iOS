...
class EmbeddedFeedbackModelViewController: UIViewController, UILimitBlockDelegate {
    ...
    override func viewDidLoad() {
        ...
        for block in (sysModel!.getForwardDictionary() as Dictionary<String, BlockDevice>).values {
            if block.type == BlockDeviceType.Block.toRaw() { // woo for effective use of post increment!
            ...
            } else if block.type == BlockDeviceType.LimitBlock.toRaw() {
                if limitBlock == nil
                {
            ...
            } else if block.type == BlockDeviceType.SysModelBlock.toRaw() {
        ...
        for block in (sysModel!.getLoopDictionary() as Dictionary<String, BlockDevice>).values {
            if(block.type == BlockDeviceType.Block.toRaw()) {
